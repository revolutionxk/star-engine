    ...
    class ImGuiVisitor {
      public:
        ...
        void operator()(const FD& desc, Value& value) const {
            if constexpr (FD::template has_attr<meta::attr::HideInEditor>())
                return;

            const bool is_ro = FD::template has_attr<meta::attr::ReadOnly>();
            if (is_ro)
                ImGui::BeginDisabled();
            ...
            if constexpr (FD::template has_attr<meta::attr::Tooltip>()) {
                if (ImGui::IsItemHovered(ImGuiHoveredFlags_DelayShort)) {
                    if (auto tt = desc.template get_attr<meta::attr::Tooltip>())
      ...
      private:
        ...
        static void draw(const FD& d, f32& v) {
            const auto sp = d.template get_attr_or<meta::attr::Speed>({0.1f}).value;
            const auto rng = d.template get_attr_or<meta::attr::Range>({});
        ...
        static void draw(const FD& d, i32& v) {
            const auto sp = d.template get_attr_or<meta::attr::Speed>({1.f}).value;
            const auto rng = d.template get_attr_or<meta::attr::Range>({});
        ...
        static void draw(const FD& d, u8& v) {
            int iv = v;
            const auto rng = d.template get_attr_or<meta::attr::Range>({0.f, 255.f});
        ...
        static void draw(const FD& d, Vector3& v) {
            if constexpr (FD::template has_attr<meta::attr::Color>()) {
            ...
            if (ImGui::DragFloat3(d.name.data(), v.data, sp, rng.min, rng.max, "%.3f")) {
                if constexpr (FD::template has_attr<meta::attr::Normalized>()) {
        ...
        static void draw(const FD& d, Quaternion& q) {
            ...
            const auto sp = d.template get_attr_or<meta::attr::Speed>({0.5f}).value;
        ...
        static void draw(const FD& d, Enum& v) {
            ...
            d.visit_enum_options([&]<std::size_t N>(const meta::attr::EnumOptions<N>& opts) {
